...
The model simulates glaciovolcanic eruptions: a subglacial volcanic eruption melting through an ice cap, forming a subsiding "ice cauldron," with the resulting meltwater draining as a glacial outburst flood (jökulhlaup).

The MATLAB source (`katlaGV`) lives in a sibling repo at `../katlaGV` (i.e. `katla/katlaGV`, alongside this `katla/gvpy` project). It is under active/ongoing development itself, not a frozen reference.

`../katlaGV/README.md`'s "General Structure" and "IceCauldron methods" sections are the authoritative reference for the model's inflow/outflow component breakdown (vent/melting, plume, ice inflow, inter-cauldron flow, supraglacial drainage, subglacial drainage) and the full method inventory of the `IceCauldron` class - consult it rather than relying on this file to enumerate that structure.

See `PROGRESS.md` for the current translation status.
...
- Standalone utility functions (i.e. not IceCauldron class methods) are consolidated into topic-based modules rather than mirrored 1:1 with MATLAB source files, split into two kinds:
  - Generic, reusable math/geometry helpers not tied to a specific component (e.g. a smooth-min blending function) go in a general module under `utilities/` (e.g. `utilities/math_helpers.py`, `utilities/geometry.py`, `utilities/validators.py`).
  - Functions specific to one of the model's major inflow/outflow components - per the MATLAB README's "General Structure" section, the model is organized around these (vent/melting, plume, ice inflow, inter-cauldron flow, supraglacial drainage, subglacial drainage) - live in their own top-level `gvpy/<component>.py` module, e.g. `gvpy/plume.py`, `gvpy/subglacial.py`, and (once translated) `gvpy/supraglacial.py` - NOT under `utilities/`. These represent major model components in their own right, not generic helpers, per explicit project direction.
- The `@IceCauldron` classdef folder translates to a single `ice_cauldron.py` module containing the `IceCauldron` class with all its methods (this does not change based on the component-based module organization above - it's specifically about standalone functions outside the class) - with one exception: plotting/output metadata methods (e.g. `getVarLabels.m`) belong with plotting tooling (a `gvpy/plotting.py` module, not yet created) rather than in `ice_cauldron.py`, since their purpose is plotting output, not model physics/state. This ties into the still-open question (see `TRANSLATION_NOTES.md`) of eventually folding such metadata into `xarray` variable `attrs` instead of a separate lookup structure.
